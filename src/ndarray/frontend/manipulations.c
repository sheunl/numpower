...
void _fillDouble(NDArray *a, double fill_value) {
    ...
    if (NDArray_DEVICE(a) == NDARRAY_DEVICE_GPU) {
#ifdef HAVE_CUBLAS
        cuda_fill_double(NDArray_DDATA(a), fill_value, NDArray_NUMELEMENTS(a));
#endif
    } else {
        for (i = 0; i < NDArray_NUMELEMENTS(a); i++) {
...
void NDArray_fillByZval(NDArray *ndarray, zval *value)
{
    ...
    } else if (NDArray_TYPE(ndarray) == NDARRAY_TYPE_DOUBLE64 && Z_TYPE_P(value) == IS_DOUBLE) {
        _fillDouble(ndarray, Z_DVAL_P(value));
    } else if (NDArray_TYPE(ndarray) == NDARRAY_TYPE_DOUBLE64 && Z_TYPE_P(value) == IS_LONG) {
        _fillDouble(ndarray, (double)Z_LVAL_P(value));
    } else if (NDArray_TYPE(ndarray) == NDARRAY_TYPE_DOUBLE64 && Z_TYPE_P(value) == IS_TRUE) {
        _fillDouble(ndarray, (double)1.0);
    } else if (NDArray_TYPE(ndarray) == NDARRAY_TYPE_DOUBLE64 && Z_TYPE_P(value) == IS_FALSE) {
